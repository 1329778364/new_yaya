	...
	<view class="container">
		...
		<view class="tui-header-box">
			<view class="tui-header" :style="{width:width+'px',height:height+'px'}">
				...
				<view class="tui-searchbox tui-search-mr" :style="{marginTop:inputTop+'px'}" @tap="search">
					...
					<!-- #endif -->
					<text class="tui-search-text" v-if="!searchKey">搜索任务</text>
		...
		<view class="tui-product-list" :style="{marginTop:px(dropScreenH+18)}">	
			<view class="tui-product-container">
				<block  v-for="(item,index) in dataList" :key="index" v-if="isList">
					<view class="tui-pro-item" :class="[isList?'tui-flex-list':'']" hover-class="hover" :hover-start-time="150" @tap="detail">
						<view class="yaya-title">{{item.taskHead}}</view>
						<view class="yaya-content-image">
							<view class="yaya-content">
								<view class="yay-info">{{item.openid}}</view>
								<view class="cu-avatar round" style="background-image">
									<open-data type="userAvatarUrl"></open-data>
								</view>
								<open-data type="userNickName" lang="zh_CN"></open-data>
								<view style="font-weight: normal;">{{item.taskDetail}}</view>
							</view>
							<image :src="'../../../static/images/mall/product/'+item.img+'.jpg'" class="tui-pro-img" :class="[isList?'tui-proimg-list':'']"
							 mode="widthFix"/>
						</view>
						<view class="yaya-bottom-func">
							<view style="margin: 0 10px; font-weight: 100 ;">{{item.Views}} 围观 </view>
							<view style="margin: 0 10px; font-weight: 100 ;">{{item.Upvotes}} 赞同 </view>
							<view style="margin: 0 10px; font-weight: 100 ;">{{item.Comments}} 评论 </view>
						</view>
					</view>
					<!--任务列表-->
				</block>
...
<script>
	...
	export default {
		...
		onLoad: function(options) {
			...
			const XYBData = wx.cloud.database().collection("xiaoyuanbang");
			XYBData.get({
